...
Also test atomic sidecar/profile publication, temp removal after each failure, exact sidecar parser, 16 MiB cap, checksum and byte count, a different origin/org namespace, newest-first offline filtering, labels retained read-only, one latest cached row per roast, stale status, LRU by `downloaded_at`, clear-unused behavior, and failure when sidecar identity does not match generated path.
...
Use `<namespace-key>/roasts/<roast-uuid>/<revision-number>-<sha256>.alog` and an adjacent `.json`; server strings never form paths. Write each artifact under `<namespace-key>/tmp/<uuid>.part`, flush/fsync, chmod files `0o600` and directories `0o700` where supported, then `os.replace` the profile before the sidecar. If sidecar publication fails, remove the newly published profile. `validate()` must re-read and strictly parse the sidecar, stream/hash the profile, and compare all identity/size/checksum fields before returning. Pruning sorts unprotected entries by downloaded time oldest first and removes sidecar/profile pairs atomically from the index view.
...
- Produces `RoastServerWorker` slots `start()`, `configure(object)`, `test_connection(str)`, `enqueue_saved(str)`, `retry_job(str)`, `remove_job(str)`, `browse(str)`, `open_online(str)`, `open_cached(str)`, `publish_staged(str)`, `clear_unused(str)`, and `stop()`; signals are those below.
