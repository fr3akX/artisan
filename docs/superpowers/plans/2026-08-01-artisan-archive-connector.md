...
Also test that persisted identity is never proof before worker-side keyring read plus exact `/auth/me`; startup offline authentication still permits exact-byte enqueue into the known namespace while remaining paused; origin/organization mismatch durably turns automatic upload off and causes zero upload; and candidate activation follows the crash-safe worker-secret sequence candidate test → durable verified pending public journal → keyring write/readback → final `/auth/me` → active disabled settings promotion/readback while journaled → pending clear/readback → worker acknowledgement → proof. Cover acknowledged live rollback in the order keyring restore/readback → public prior-state restore/readback while journaled → journal clear, with rollback failure retaining the marker. Cover shutdown/restart at pre-keyring, post-keyring, post-final-auth, post-promotion/pre-ack, and post-ack cuts without shutdown rollback, using organization A/B jobs and zero upload before exact proof. Test QSettings `AccessError`, `FormatError`, and fresh-readback mismatch, fixed failure text, and no processing.
...
Construct one `QThread` and one `RoastServerWorker`, connect commands with explicit queued connections, move the worker, and only then start. Share a secret-free `ConfigurationFence` whose exact-generation permits linearize startup `/auth/me`, exact-latest-request candidate `/auth/me`, retained-transaction final `/auth/me`, authorization installation, activation installation, namespace resume, lease transition, lease-capable timer arm, `post_aroast`, `upload_revision`, and online list/detail/download calls against synchronous revocation. Acquire at the immediate operation boundary, hold no fence lock across the operation, recheck/discard authentication results after authority changes, and release when that one bounded operation returns. A permit acquired first is a bounded in-flight operation under its prior exact authority; revocation or candidate cancellation wins against every later acquisition. Add deterministic before-permit barriers at every named boundary and assert zero side effect after revocation/cancellation, plus active-permit tests for grandfathered work.

A persisted public identity configures a known namespace but installs no controller proof and authorizes no lease: the worker pauses it, reads keyring, calls `/auth/me`, and resumes only after exact canonical origin and persisted identity match. Retry transient authentication with work paused; on mismatch emit only the fixed credential failure and persist automatic upload false. Candidate credentials remain in the redacted worker transaction with the old credential; Qt carries opaque IDs only. Keep pending public settings durable through keyring commit and final `/auth/me`; promote/read back active-disabled settings while the journal remains, clear/read back pending, and only then acknowledge. Interactive cancellation waits asynchronously for explicit worker keyring-rollback completion before restoring public state and clearing pending; a bounded status timeout and any rollback failure leave delivery paused and the journal intact. Shutdown interrupts/revokes but never starts cross-store rollback; worker stop scrubs transaction secret references without changing committed keyring state. Recover every pending/promoted cut paused and validate exact identity before delivery; an absent credential for the pending target definitively identifies the pre-keyring-commit cut even when a different active origin/identity exists, so clear the verified journal and leave that prior active connection disabled until startup validation.

`saved_profile(serialized_profile, profile, modified_at)` returns immediately when disabled/auto-off; otherwise validate only `bytes` length `1..16 MiB`, detach `ProfileData`, canonicalize the aware timestamp, vault that immutable request, and emit only its ID. Manual upload receives the same serializer-owned triple. The worker calls `Outbox.snapshot_bytes()`, projects metadata from the matching profile/timestamp, enqueues, then clears byte/profile references. Neither controller path reads nor worker reopens a mutable source path. On `downloadStaged`, call the injected Artisan validator in the UI thread while the staged file is still hidden; on success vault a publish command, and on validator failure vault a `discard_staged` command before emitting `INVALID_RESPONSE`. Controller cancellation, opener exceptions, and vault failures must choose the same explicit discard command and must never unlink the path directly. Only after `cachePublished` or `cachedReady` emit `profileReady(str(path), source)`; after `ApplicationWindow.openRoastServerProfile()` reports success through `record_open_source()`, queue the immutable current protected set immediately. Queue the matching set after local transition and every namespace configuration; worker uses only its latest set for every publication prune and clear. `shutdown()` requests interruption before synchronous permit revocation, emits stop, and waits at most 15 seconds without restoring settings or keyring; worker `stopped` directly schedules `deleteLater` and thread quit after timer/stages/in-memory secrets/stores close, never using `terminate()`.
