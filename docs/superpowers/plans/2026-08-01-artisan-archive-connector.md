...
Add causal cleanup tests that replace a stage pathname and add a hard-link alias, inject the first unlink failure, and prove every later sidecar/profile/publication-temp/stage cleanup is still attempted while neither the replacement pathname nor other hard links are removed. Add real subprocess barriers proving an arbitrarily old active stage survives maintenance, explicit owner discard removes its pair, a process crash makes its pair collectible, and two first-root publishers targeting the same destination both complete with one valid pair. Add portable complete-flow native seams and `@pytest.mark.win32` runtime tests for cache ACLs, lock contention, reparse/junction containment, replacement, file/directory flush failures, and deletion; non-Windows runs must report native Windows tests as skipped rather than passed.
